    ...
    <div>
      <div class="bg-header py-1 flex justify-between">
        <div class="flex items-center">
          <img src="/cis/logo-track.png" alt="" class="h-6 px-5">
        </div>
        <div class="w-1/3 flex justify-center items-center text-gray-800 text-2xl font-bold">
          <AppButton type="transparent" @click="$router.push({ path: '/target' })"
            :is-active="$route.path === '/target'" class="w-1/3 !text-2xl">
            {{ $t("Target") }}
          </AppButton>
          <AppButton type="transparent" @click="onClickInvest" :is-active="$route.path === '/investigation'"
       ...
       text-default">
          <div class="relative flex items-center gap-1 pr-6 text-xl cursor-pointer" @mouseenter="onMouseEnter"
            @mouseleave="onMouseLeave">
            <AppSvgIcon name="icon-user" :color="'#b5bec0'" class="w-8 h-8" />
            {{ user.username }}
            <div
              class="absolute w-32 z-10 h-12 dialog-hint text-center bg-ctrl-secondary py-3 rounded hover:bg-ctrl-secondary-hover"
              style="top: 100%; left: 50%; transform: translate(-50%, 0);" @click="onLogout" v-if="hover">
              {{ $t("Logout") }}
            </div>
          </div>
          <div class="flex">
